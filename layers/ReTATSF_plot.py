...
def visualize_first_sample(target_series):
    # target_series: [B, C_T, L]
    sample = target_series[0]  # [C_T, L]
    C_T, L = sample.shape

    plt.figure(figsize=(12, 2 * C_T))
    for i in range(C_T):
        plt.subplot(C_T, 1, i + 1)
        plt.plot(sample[i].cpu().numpy())
        plt.title(f"Channel {i+1}")
        plt.xlabel("Time")
        plt.ylabel("Value")
        plt.tight_layout()
    plt.savefig('./fig/series.pdf', format='pdf', bbox_inches='tight')
    plt.show()

def visualize_text_embedding_heatmap(text_emb):
    """
    text_emb: [B, C_T*(K_T+1), L+H, D]
    可视化第一个 batch 中第一个通道的 embedding 热图
    """
    emb = text_emb[0, 0]  # [L+H, D]
    emb = emb.cpu().detach().numpy().T  # 转置为 [D, L+H]，行是 embedding 维度，列是时间维度

    plt.figure(figsize=(12, 6))
    sns.heatmap(emb, cmap='viridis', cbar=True)
    plt.title("Embedding Heatmap of First Batch, First Channel")
    plt.xlabel("Token Position (L+H)")
    plt.ylabel("Embedding Dimension (D)")
    plt.tight_layout()
    plt.savefig('./fig/embeddding.pdf', format='pdf', bbox_inches='tight')
    plt.show()
...
class TS_CoherAnalysis(nn.Module):
    ...
    def forward(self, target_series, TS_database):
        ...
        visualize_first_sample(target_series.cpu())
...
class CrossandOutput(nn.Module):
    ...
    def forward(self, text_emb, temp_emb):#text_emb[B, C_T*K_n, H, D] temp_emb[B, C_T*(K_T+1), L, D]
        ...
        visualize_text_embedding_heatmap(text_emb)
