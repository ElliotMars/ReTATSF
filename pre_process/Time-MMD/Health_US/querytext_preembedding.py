...
df = pd.read_parquet('../../../dataset/Time-MMD/numerical/Health_US/Health_US.parquet')
df['start_date'] = pd.to_datetime(df['start_date'])
df['end_date'] = pd.to_datetime(df['end_date'])
time_span = df[['start_date', 'end_date']].values.tolist()

des = pd.read_parquet('../../../dataset/Time-MMD/textual/Health_US/QueryTextPackage.parquet')

if ForecastingPoint:
    save_dir = f'../../../dataset/Time-MMD/textual/Health_US/QueryText-embedding-{BERT_model}-ForecastingPoint/{target_id}'
    ...
    save_dir = f'../../../dataset/Time-MMD/textual/Health_US/QueryText-embedding-{BERT_model}-Description/{target_id}'
    os.makedirs(save_dir, exist_ok=True)
    qt = (
